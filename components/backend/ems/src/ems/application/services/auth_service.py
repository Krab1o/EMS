...
class AuthService:
    ...
    async def register_student(
        self, body: dto.UserCreateRequest
        ...
        if (
            not AuthService.check_password(body.password)
            or not AuthService.check_email(body.email)
            or body.telegram is not None
            and not AuthService.check_telegram(body.telegram)
            or body.vk is not None
            and not AuthService.check_vk(body.vk)
            or body.phone_number is not None
            and not AuthService.check_phone(body.phone_number)
        ):
