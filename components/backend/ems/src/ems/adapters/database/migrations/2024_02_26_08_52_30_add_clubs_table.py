...
def upgrade():
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["club_id"],
            ["clubs.id"],
            name=op.f("fk_users_favorite_clubs_club_id_clubs"),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["user_id"],
            ["users.id"],
            name=op.f("fk_users_favorite_clubs_user_id_users"),
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint(
            "user_id", "club_id", name=op.f("pk_users_favorite_clubs")
        ),
    ...
    op.add_column(
        "clubs",
        sa.Column("vk", sa.String(length=256), nullable=True, comment="VK"),
    )
    op.add_column(
        "clubs",
        sa.Column(
            "youtube", sa.String(length=256), nullable=True, comment="YouTube"
        ),
    )
    op.add_column(
        "clubs",
        sa.Column(
            "rutube", sa.String(length=256), nullable=True, comment="Rutube"
        ),
    )
    op.add_column(
        "clubs",
        sa.Column(
            "tiktok", sa.String(length=256), nullable=True, comment="TikTok"
        ),
    )
    op.alter_column(
        "clubs",
