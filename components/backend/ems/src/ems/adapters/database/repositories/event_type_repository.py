...
class EventTypeRepository:
    ...
    async def get_list(self, page: int, size: int) -> list[entities.EventType]:
        query = select(entities.EventType).offset(page * size).limit(size)
        async with self.async_session_maker() as session:
    ...
    async def delete_one(self, event_type_id: int):
        query = delete(entities.EventType).where(
            entities.EventType.id == event_type_id
        )
        async with self.async_session_maker() as session:
